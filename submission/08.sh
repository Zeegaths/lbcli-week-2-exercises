...
# Step 5: Create the raw transaction with RBF enabled
# To enable RBF, set sequence number to less than 0xFFFFFFFF-1 (4294967294)
# Common value is 0xFFFFFFFD (4294967293)
rbf_sequence=4294967293

echo "Creating raw transaction with RBF enabled..."
raw_tx_hex=$(bitcoin-cli -regtest createrawtransaction \
    '[{"txid":"'$txid'","vout":'$utxo_vout',"sequence":'$rbf_sequence'}]' \
    '{"'$recipient'":'$amount_btc',"'$change_address'":'$change_btc'}')

echo "Raw Transaction (RBF-enabled): $raw_tx_hex"
